...
function! s:ExtendTagItemToPopupItem(tagItem, szTypeName)
    ...
    " If a member, append it's type to the menu
    if index(['m'], tagItem.kind[0]) >= 0
        let mType = strpart(tagItem.cmd, match(tagItem.cmd, '\w'))
        let mType = strpart(mType, 0, match(mType, ';'))
        let mType = substitute(mType, '\w\+$', '', 'g')
        if mType[strlen(mType)-1] == ' '
            let mType = strpart(mType, 0, strlen(mType)-1)
        endif
        let szItemMenu = szItemMenu.' ('.mType.')'
    endif
